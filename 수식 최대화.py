...
def solution(expression):
    ...
    for p in permutations((1, 2, 3), 3):
        ...
        postfix = [] # postfix 형태의 수식을 담는 리스트
        stack = [] # 연산자들을 담는 스택
        i = 0 # expression 순회에 쓰일 인덱스
        
        # infix 형태의 수식을 postfix로 바꿔 postfix 리스트에 넣음
        for o in operation_index:
            # 숫자부분을 postfix에 넣음
            postfix.append(int(expression[i:o]))
        ...
        # 마지막 연산자 뒤의 숫자 처리
        postfix.append(int(expression[operation_index[-1]+1:]))
